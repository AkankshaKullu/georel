...
class SessionState extends EventEmitter {
    ...
    constructor(connection, endpoint) {
         ...
         * @private
         */
        this._nextOutgoingID = /** @type{TransferNumber}*/ (1);
         ...
         * @private
         */
        this._nextIncomingID = /** @type{TransferNumber}*/ (0);

        /**
         * @type {boolean}
         * @private
         */
        this._nextIncomingIDWeak = false;
         ...
         * @private
         */
        this._remoteOutgoingFlowAt = /** @type{UInt}*/ (connection.options.minRemoteOutgoingWindow);
    ...
    onSendBegin(frame) {
        ...
        this._remoteOutgoingWindow = this._incomingWindow;
    ...
    onPeerBegin(frame) {
        ...
        this._nextIncomingIDWeak = true; // wait for first transfer, AMQP Net Lite: 4294967293, but first delivery 0 and protonj 1, but first delivery 0
    ...
    onPeerAttach(frame) {
        ...
        if (ls.outgoingHandle > -1) this._onSendDrain();
    }
    ...
    onSendFlow(frame = new Flow()) {
        ...
        this._remoteOutgoingWindow = frame.incomingWindow;
    ...
    onPeerFlow(frame) {
        ...
        if (this._remoteIncomingPause && this._remoteIncomingWindow > 0) {
            this._remoteIncomingPause = false;
            if (this._outgoingStreamPause === false && this._outgoingWindowPause === false) this._onSendDrain();
        }
    ...
    onSendDisposition(frame) {
        if (frame.settled) {
            ...
            if (this._remoteOutgoingWindow <= this._remoteOutgoingFlowAt) {
                this.onSendFlow();
            }
        }
        if (this.outgoingHandle < 0) {
    ...
    onPeerDisposition(frame) {
        ...
        if (frame.roleReceiver && this._outgoingWindowPause && this._outgoingUnsettled.size === 0) {
            this._outgoingWindowPause = false;
            if (this._remoteIncomingPause === false && this._outgoingStreamPause === false) this._onSendDrain();
        }
    }
     ...
     * @param {!Transfer} frame
     */
    _onPushTransfer(frame) {
        this._remoteIncomingWindow = /** @type{UInt} */ (Math.max(0, this._remoteIncomingWindow - 1));
        frame.deliveryID = this._nextOutgoingID;
        this._nextOutgoingID = SerialNumberUInt.increment(this._nextOutgoingID, 1);
        if (!frame.settled)
            this._outgoingUnsettled.set(frame.deliveryID, frame);
        frame.channel = this.outgoingHandle;
    }
    ...
    _onSendStreamDrain() {
        if (this._outgoingStreamPause === true) {
            this._outgoingStreamPause = false;
            if (this._outgoingWindowPause === false && this._remoteIncomingPause === false) this._onSendDrain();
        }
    }
    ...
    onPeerTransfer(frame) {
        if (this._nextIncomingID !== frame.deliveryID) {
            if (this._nextIncomingIDWeak) {    // learn next incoming ID from first transfer after inconsistent peer begin performative
                this._nextIncomingIDWeak = false;
                this._nextIncomingID = frame.deliveryID;
            } else {
                throw ErrMsg(EC.INCOMING_WINDOW_SEQUENCE, this.incomingHandle, frame.linkHandle, frame.deliveryID, this._nextIncomingID);
            }
        }
        this._nextIncomingID = SerialNumberUInt.increment(this._nextIncomingID, 1);
        this._remoteOutgoingWindow -= /** @type{UInt} */ (1);
        if (!frame.settled) {
        ...
        } else if (this._remoteOutgoingWindow <= this._remoteOutgoingFlowAt) {
            this.onSendFlow();
        }
